...
너의 목표는 각 사진 파일에 포함된 모든 내용을 마크다운 `코드`로 보내는 것이다.
텍스트가 있다면 텍스트 원문 그대로를, 이미지가 있다면 이미지에 대한 설명을 적어줘. 사진 파일 이름은 적지 않아도 돼. 모든 답을 하나의 마크다운 코드로 적어주고, 답변 이외의 아무 말도 하지 마.
...
PDF에서 `PyMuPDF`로 추출한 슬라이드별 텍스트가 있다면 보내줄건데, 사진 파일에서 인식한 내용을 중심으로 작성한다. PyMuPDF 추출텍스트는 단순한 참고사항이다. 이미지로만 파악하기 힘들 때 사용하고, 넣지 않아도 된다.
"""
